...
Game.Launch = function () {
	...
	*/
	...
	Game.Init = function () {
		...
		l('links').innerHTML =
			(Game.beta
				? '<a href="../" target="blank">Live version</a> | '
				: '<a href="beta" target="blank">Try the beta!</a> | ') +
		...
		Game.WriteSave = function (
			...
			for (var i in Game.Upgrades) { //upgrades
				...
				toCompress.push(
					Math.min(me.unlocked, 1),
					Math.min(me.bought, 1)
				);
			}
			...
			if (exporting) {
				...
				if (Game.useLocalStorage) {
					...
					if (!window.localStorage.getItem(Game.SaveTo))
						Game.Popup(
							'Error while saving.<br>Export your save instead!'
						);
				...
				if (str != '') {
					...
					if (version >= 1 && version > Game.version) {
						...
						for (var i in Game.ObjectsById) {
							...
							me.setSpecial(0);
							if (me.special && me.specialUnlocked == 1)
								me.special();
							me.refresh();
						}
						...
						*/
						...
						if (Game.mobile || Game.Has('Perfect idling')) {
							var amount =
								((new Date().getTime() - Game.lastDate) /
									1000) *
								Game.cookiesPs;
							if (amount > 0) {
								Game.Popup(
			...
			if (
				...
				if (!bypass) {
					...
					if (Game.prestige.ready)
						prestige = Game.prestige['Heavenly chips'];
					Game.prestige = [];
		...
		Game.CalculateGains = function () {
			...
			for (var i in Game.Upgrades) {
				var me = Game.Upgrades[i];
				if (me.bought > 0) {
					if (me.type == 'cookie' && Game.Has(me.name))
						mult += me.power * 0.01;
				}
			}
			...
			mult +=
				parseFloat(Game.prestige['Heavenly chips']) *
				0.02 *
				heavenlyMult;
			...
			for (var i = 0; i < Game.cpsAchievs.length / 2; i++) {
				if (rawCookiesPs >= Game.cpsAchievs[i * 2 + 1])
		...
		Game.goldenCookie.spawn = function () {
			if (this.time < this.minTime || this.life > 0)
			...
			if (Game.season == 'halloween' && this.wrath)
				r = Math.floor(Math.random() * 36 - 18);
			if (Game.season == 'valentines') {
				me.style.background = 'url(img/hearts.png)';
				me.style.backgroundPosition =
					-(Math.floor(Math.random() * 8) * 96) + 'px 0px';
				r = Math.floor(Math.random() * 36 - 18);
				if (this.wrath) r += 180;
			}
			...
			this.x =
				Math.floor(
					Math.random() *
						Math.max(0, screen.right - 300 - screen.left - 128) +
						screen.left +
						64
				) - 64;
			this.y =
				Math.floor(
					Math.random() *
						Math.max(0, screen.bottom - screen.top - 128) +
						screen.top +
						64
				) - 64;
		...
		Game.goldenCookie.update = function () {
			...
			if (this.life > 0) {
				...
				l('goldenCookie').style.opacity =
					1 -
					Math.pow((this.life / (Game.fps * this.dur)) * 2 - 1, 4);
		...
		Game.goldenCookie.choose = function () {
			...
			if (this.wrath > 0 && Math.random() < 0.3)
		...
		Game.goldenCookie.click = function () {
			...
			if (me.life > 0) {
				...
				if (choice == 'frenzy') {
					...
					Game.recalculateGains = 1;
					Game.Popup(
						'Frenzy : cookie production x7 for ' +
							time +
							' seconds!'
					);
				} else if (choice == 'multiply cookies') {
					var moni =
						Math.min(Game.cookies * 0.1, Game.cookiesPs * 60 * 20) +
						13; //add 10% to cookies owned (+13), or 20 minutes of cookie production - whichever is lowest
					Game.Earn(moni);
					...
					var moni =
						...
						) + 13; //lose 5% of cookies owned (-13), or 10 minutes of cookie production - whichever is lowest
					...
					Game.recalculateGains = 1;
					Game.Popup(
						'Elder frenzy : cookie production x666 for ' +
							time +
							' seconds!'
					);
				} else if (choice == 'clot') {
					...
					Game.recalculateGains = 1;
					Game.Popup(
						'Clot : cookie production halved for ' +
							time +
							' seconds!'
					);
					...
					Game.recalculateGains = 1;
					Game.Popup(
						'Click frenzy! Clicking power x777 for ' +
							time +
							' seconds!'
					);
					...
					if (
						(Math.random() < 0.01 ||
							nextMoni >= Game.cookies * 0.25 ||
							nextMoni > Game.cookiesPs * 60 * 60 * 6) &&
						me.chain > 4
					) {
						//break the chain if we're above 5 digits AND it's more than 25% of our bank, it grants more than six hours of our CpS, or just a 1% chance each digit
						me.chain = 0;
						Game.Popup(
							'Cookie chain : +' +
								Beautify(moni) +
								' cookies!<br>Cookie chain over.'
						);
					} else
						Game.Popup(
							'Cookie chain : +' + Beautify(moni) + ' cookies!'
						);
					Game.Earn(moni);
				} else if (choice == 'blab') {
			...
			this.y =
				Math.floor(
					Math.random() *
						Math.max(
							0,
							this.bounds.bottom - this.bounds.top - 256
						) +
						this.bounds.top +
						128
				) - 128;
		...
		Game.seasonPopup.update = function () {
			...
			if (this.life > 0) {
				...
				me.style.opacity =
					1 -
					Math.pow((this.life / (Game.fps * this.dur)) * 2 - 1, 12);
				this.x =
					Math.floor(
						(1 - this.life / (Game.fps * this.dur)) *
				...
				me.style.top =
					this.y -
					Math.floor(
						Math.abs(Math.pow(Math.sin(this.life * 0.05), 1)) * 128
					) +
					'px';
		...
		Game.particleAdd = function (x, y, xd, yd, size, dur, z, pic, text) {
			...
			if (1 || Game.prefs.particles) {
				...
				if (Game.LeftBackground && !auto)
					x = Math.floor(
						Math.random() * Game.LeftBackground.canvas.width
					);
				var y = y || -64;
				var me = Game.particles[i];
				me.life = 0;
		...
		Game.textParticlesUpdate = function () {
			...
			for (var i in Game.textParticles) {
				...
				if (me.life != -1) {
					...
					var y =
						me.y -
						(1 - Math.pow(1 - me.life / (Game.fps * 4), 10)) * 50;
					//me.y=me.life*0.25+Math.random()*0.25;
					me.life++;
					var el = me.l;
		...
		Game.WriteSelector = function (
			prefName,
			button,
			list,
			def,
			selected,
			callback //def is default (which is a reserved keyword)
		) {
			var str = '';
			var me = 0;
			for (var i in list) {
				...
				str +=
					...
					'" ' +
					Game.getTooltip(
						'<div style="min-width:200px;"><div class="name" style="text-align:center;">' +
							me.name +
		...
		Game.Select = function (prefName, button, value) {
			if (l(button + '-' + Game[prefName]))
				l(button + '-' + Game[prefName]).className = 'crate';
			if (l(button + '-' + value))
				l(button + '-' + value).className = 'crate enabled';
			Game[prefName] = value;
		};
		...
		Game.UpdateMenu = function () {
			...
			if (Game.onMenu != '') {
				str +=
					'<div style="position:absolute;top:8px;right:8px;cursor:pointer;font-size:16px;" onclick="Game.ShowMenu();">X</div>';
			}
			if (Game.onMenu == 'prefs') {
				str +=
					...
					'<label>(the game will ask you to confirm when you close the window)</label></div>' +
						...
						? '<div class="title">Customization</div>' +
						  ...
						  Game.WriteSelector(
								...
								Game.backgroundType > 0
									? Game.UpgradesById[Game.backgroundType]
											.name
									: Game.Upgrades['Blue background'].name
						  ) +
						  '</div>'
						: '') +
				...
				//might want to make this static someday
				str += Game.updateLog;
				...
				for (var i in list) {
					var me = list[i];
					if ((!me.disabled && me.hide != 3) || me.won > 0)
						achievementsTotal++;
				...
				if (Game.Has('A festive hat')) {
					for (var i = 0; i <= Game.santaLevel; i++) {
						santaStr +=
							...
							' style="width:96px;height:96px;margin:2px;float:left;background:url(img/santa.png) ' +
							-i * 96 +
							'px 0px;"></div>';
					}
				...
				var startDate = Game.sayTime(
					(date.getTime() / 1000) * Game.fps,
					2
				);
				date.setTime(new Date().getTime() - Game.fullDate);
				var fullDate = Game.sayTime(
					(date.getTime() / 1000) * Game.fps,
					2
				);
				...
				str +=
					...
					'</div></div>' +
					'<div class="listing"><b>Cookies baked (this game) :</b> <div class="price plain">' +
					...
					(Game.resets
						? '<div class="listing"><b>Legacy started :</b> ' +
						  ...
						  '</div>'
						: '') +
					'<div class="listing"><b>Session started :</b> ' +
					startDate +
					' ago</div>' +
					...
					(Game.prestige['Heavenly chips'] > 0
						? '</div><div class="subsection">' +
						  ...
						  Beautify(Game.prestige['Heavenly chips']) +
						  ' heavenly chip' +
		...
		Game.getNewTicker = function () {
					...
					list.push(
						'News : ' +
							choose([
								...
								'cookies unexpectedly popular among ' +
									choose(animals) +
									'!',
			...
			if (
				Game.elderWrath > 0 &&
				(Game.pledges == 0 || Math.random() < 0.5)
			) {
				list = [];
			...
			if (Game.Ticker != '') {
				if (Game.TickerAge < Game.fps * 1 && 1 == 2) {
					...
					str = str.substring(
						0,
						str.length *
							Math.min(1, Game.TickerAge / (Game.fps * 1))
					);
		...
		Game.Object = function (
			...
			this.getPrice = function () {
				var price =
					this.basePrice * Math.pow(Game.priceIncrease, this.amount);
		...
		Game.NewDrawFunction = function (
			...
			return function () {
				...
				if (this.drawSpecialButton && this.specialUnlocked) {
					...
					l('rowSpecialButton' + this.id).innerHTML =
		...
		Game.RebuildStore = function () //redraw the store from scratch
		{
			...
			for (var i in Game.Objects) {
				...
				str +=
					...
					me.icon +
					'Off.png);"></div><div class="icon" id="productIcon' +
					me.id +
					'" style="background-image:url(img/' +
					me.icon +
			...
			function () {
				return Game.ComputeCps(
					...
					Game.Has('Time paradox resolver') +
						...
						Game.Has('Yestermorrow comparators')
				);
			},
		...
		new Game.Object(
			...
			function () {
				return Game.ComputeCps(
					10000000,
					Game.Has('Gem polish') * 1000000,
					Game.Has('9th color') +
						Game.Has('Chocolate light') +
						Game.Has('Grainbow') +
						Game.Has('Pure cosmic light')
				);
			},
			Game.NewDrawFunction(0, 16, 4, 64, 1, 20),
			function () {
		...
		Game.RebuildUpgrades =
			function () //recalculate the upgrades you can buy
			{
		...
		new Game.Upgrade(
			'Cheap hoes',
			'Farms gain <b>+1</b> base CpS.<q>Rake in the dough!</q>',
		...
		new Game.Upgrade(
			...
			"Factories gain <b>+4</b> base CpS.<q>You're going places.</q>",
		...
		new Game.Upgrade(
			'Vanilla nebulae',
			"Shipments gain <b>+30</b> base CpS.<q>If you removed your space helmet, you could probably smell it!<br>(Note : don't do that.)</q>",
		...
		new Game.Upgrade(
			...
			'Cookie production multiplier <b>+5%</b>.<q>Get yourself some jam cookies!</q>',
			99999999,
			[1, 3]
		);
		new Game.Upgrade(
		...
		new Game.Upgrade(
			'Coconut cookies',
			'Cookie production multiplier <b>+5%</b>.<q>These are *way* flaky.</q>',
			999999999,
			[3, 3]
		);
		new Game.Upgrade(
			'White chocolate cookies',
			"Cookie production multiplier <b>+5%</b>.<q>I know what you'll say. It's just cocoa butter! It's not real chocolate!<br>Oh please.</q>",
			999999999,
			[4, 3]
		);
		new Game.Upgrade(
			'Macadamia nut cookies',
			"Cookie production multiplier <b>+5%</b>.<q>They're macadamn delicious!</q>",
			999999999,
			[5, 3]
		);
		power = 10;
		new Game.Upgrade(
			'Double-chip cookies',
			'Cookie production multiplier <b>+10%</b>.<q>DOUBLE THE CHIPS<br>DOUBLE THE TASTY<br>(double the calories)</q>',
			99999999999,
			[6, 3]
		);
		power = 5;
		new Game.Upgrade(
			'Sugar cookies',
			'Cookie production multiplier <b>+5%</b>.<q>Tasty, if a little unimaginative.</q>',
			99999999,
			[7, 3]
		);
		power = 10;
		new Game.Upgrade(
			...
			"Cookie production multiplier <b>+10%</b>.<q>Orteil's favorite.</q>",
			99999999999,
			[8, 3]
		);
		new Game.Upgrade(
			'All-chocolate cookies',
			'Cookie production multiplier <b>+10%</b>.<q>CHOCOVERDOSE.</q>',
			99999999999,
			[9, 3]
		);
		type = '';
		...
		new Game.Upgrade(
			...
			'Cookie production multiplier <b>+15%</b>.<q>These absorb light so well you almost need to squint to see them.</q>',
			999999999999,
			[10, 3]
		);
		new Game.Upgrade(
			'White chocolate-coated cookies',
			'Cookie production multiplier <b>+15%</b>.<q>These dazzling cookies absolutely glisten with flavor.</q>',
			999999999999,
			[11, 3]
		);
		type = '';
		...
		new Game.Upgrade(
			'Farmer grandmas',
			'Grandmas are <b>twice</b> as efficient.<q>A nice farmer to grow more cookies.</q>',
			Game.Objects['Farm'].basePrice * tier2,
			[10, 9],
			function () {
		...
		new Game.Upgrade(
			'Worker grandmas',
			'Grandmas are <b>twice</b> as efficient.<q>A nice worker to manufacture more cookies.</q>',
			Game.Objects['Factory'].basePrice * tier2,
			[10, 9],
			function () {
				Game.Objects['Grandma'].drawFunction();
		...
		new Game.Upgrade(
			'Miner grandmas',
			'Grandmas are <b>twice</b> as efficient.<q>A nice miner to dig more cookies.</q>',
			Game.Objects['Mine'].basePrice * tier2,
			[10, 9],
			function () {
				Game.Objects['Grandma'].drawFunction();
		...
		new Game.Upgrade(
			'Cosmic grandmas',
			'Grandmas are <b>twice</b> as efficient.<q>A nice thing to... uh... cookies.</q>',
			Game.Objects['Shipment'].basePrice * tier2,
			[10, 9],
			function () {
				Game.Objects['Grandma'].drawFunction();
		...
		new Game.Upgrade(
			'Transmuted grandmas',
			'Grandmas are <b>twice</b> as efficient.<q>A nice golden grandma to convert into more cookies.</q>',
			Game.Objects['Alchemy lab'].basePrice * tier2,
			[10, 9],
			function () {
				Game.Objects['Grandma'].drawFunction();
		...
		new Game.Upgrade(
			'Altered grandmas',
			'Grandmas are <b>twice</b> as efficient.<q>a NiCe GrAnDmA tO bA##########</q>',
			Game.Objects['Portal'].basePrice * tier2,
			[10, 9],
			function () {
				Game.Objects['Grandma'].drawFunction();
		...
		new Game.Upgrade(
			"Grandmas' grandmas",
			"Grandmas are <b>twice</b> as efficient.<q>A nice grandma's nice grandma to bake double the cookies.</q>",
		...
		new Game.Upgrade(
			'One mind',
			'[Research]<br>Each grandma gains <b>+1 base CpS for every 50 grandmas</b>.<div class="warning">Note : the grandmothers are growing restless. Do not encourage them.</div><q>We are one. We are many.</q>',
			160000000000,
			[4, 9],
			function () {
				Game.elderWrath = 1;
		...
		new Game.Upgrade(
			'Communal brainsweep',
			'[Research]<br>Each grandma gains another <b>+1 base CpS for every 50 grandmas</b>.<div class="warning">Note : proceeding any further in scientific research may have unexpected results. You have been warned.</div><q>We fuse. We merge. We grow.</q>',
			640000000000,
			[6, 9],
			function () {
				Game.elderWrath = 2;
		...
		new Game.Upgrade(
			'Elder Pact',
			'[Research]<br>Each grandma gains <b>+1 base CpS for every 20 portals</b>.<div class="warning">Note : this is a bad idea.</div><q>squirm crawl slither writhe<br>today we rise</q>',
			2560000000000,
			[8, 9],
			function () {
				Game.elderWrath = 3;
		...
		new Game.Upgrade(
			'Elder Pledge',
			'[Repeatable]<br>Contains the wrath of the elders, at least for a while.<q>This is a simple ritual involving anti-aging cream, cookie batter mixed in the moonlight, and a live chicken.</q>',
			1,
			[9, 9],
			function () {
				Game.elderWrath = 0;
				...
				Game.pledgeT =
					Game.fps *
					60 *
					(Game.Has('Sacrificial rolling pins') ? 60 : 30);
				Game.Upgrades['Elder Pledge'].basePrice = Math.pow(
					8,
					Math.min(Game.pledges + 2, 14)
				);
		...
		new Game.Upgrade(
			'Zebra cookies',
			'Cookie production multiplier <b>+15%</b>.<q>...</q>',
			9999999999999,
			[1, 4]
		);
		type = '';
		...
		new Game.Upgrade(
			'Elder Covenant',
			"[Switch]<br>Puts a permanent end to the elders' wrath, at the price of 5% of your CpS.<q>This is a complicated ritual involving silly, inconsequential trivialities such as cursed laxatives, century-old cacao, and an infant.<br>Don't question it.</q>",
			66666666666666,
			[8, 9],
			function () {
				Game.pledgeT = 0;
		...
		new Game.Upgrade(
			...
			'[Switch]<br>You will get 5% of your CpS back, but the grandmatriarchs will return.<q>we<br>rise<br>again</q>',
			6666666666,
			[8, 9],
			function () {
		...
		new Game.Upgrade(
			...
			"Elder pledge last <b>twice</b> as long.<q>These are mostly for spreading the anti-aging cream.<br>(and accessorily, shortening the chicken's suffering.)</q>",
			2888888888888,
			[2, 9]
		);
		...
		new Game.Upgrade(
			'Snickerdoodles',
			'Cookie production multiplier <b>+15%</b>.<q>True to their name.</q>',
			99999999999999,
			[2, 4]
		);
		new Game.Upgrade(
		...
		new Game.Upgrade(
			'Macaroons',
			'Cookie production multiplier <b>+15%</b>.<q>Not to be confused with macarons.<br>These have coconut, okay?</q>',
			99999999999999,
			[4, 4]
		);
		type = '';
		...
		new Game.Upgrade(
			'Empire biscuits',
			'Cookie production multiplier <b>+15%</b>.<q>For your growing cookie empire, of course!</q>',
			99999999999999,
			[5, 4]
		);
		new Game.Upgrade(
		...
		new Game.Upgrade(
			...
			'Cookie production multiplier <b>+15%</b>.<q>Yes, quite.</q>',
			99999999999999,
			[7, 4]
		);
		new Game.Upgrade(
			'Round british tea biscuits',
			'Cookie production multiplier <b>+15%</b>.<q>Yes, quite riveting.</q>',
			99999999999999,
			[8, 4]
		);
		new Game.Upgrade(
			'Round chocolate british tea biscuits',
			'Cookie production multiplier <b>+15%</b>.<q>Yes, quite riveting indeed.</q>',
			99999999999999,
			[9, 4]
		);
		new Game.Upgrade(
			'Round british tea biscuits with heart motif',
			'Cookie production multiplier <b>+15%</b>.<q>Yes, quite riveting indeed, old chap.</q>',
			99999999999999,
			[10, 4]
		);
		new Game.Upgrade(
			'Round chocolate british tea biscuits with heart motif',
			'Cookie production multiplier <b>+15%</b>.<q>I like cookies.</q>',
			99999999999999,
			[11, 4]
		);
		type = '';
		...
		new Game.Upgrade(
			'Sugar bosons',
			'Antimatter condensers gain <b>+99,999</b> base CpS.<q>Sweet firm bosons.</q>',
		...
		new Game.Upgrade(
			'Antigrandmas',
			'Grandmas are <b>twice</b> as efficient.<q>A mean antigrandma to vomit more cookies.<br>(Do not put in contact with normal grandmas; loss of matter may occur.)</q>',
			Game.Objects['Antimatter condenser'].basePrice * tier2,
			[10, 9],
			function () {
				Game.Objects['Grandma'].drawFunction();
		...
		new Game.Upgrade(
			'Palmiers',
			'Cookie production multiplier <b>+20%</b>.<q>Palmier than you!<br>Yes, I just said that.</q>',
			199999999999999,
			[13, 3]
		);
		new Game.Upgrade(
			'Palets',
			"Cookie production multiplier <b>+20%</b>.<q>You could probably play hockey with these.<br>I mean, you're welcome to try.</q>",
			199999999999999,
			[12, 4]
		);
		new Game.Upgrade(
			'Sabl&eacute;s',
			"Cookie production multiplier <b>+20%</b>.<q>The name implies they're made of sand. But you know better, don't you?</q>",
			199999999999999,
			[13, 4]
		);
		type = '';
		...
		new Game.Upgrade(
			'Sextillion fingers',
			'The mouse and cursors gain <b>+200</b> cookies for each non-cursor object owned.<q>sometimes<br>things just<br>click</q>',
			500000000000000,
			[12, 13]
		);
		...
		new Game.Upgrade(
			...
			Game.Objects['Grandma'].basePrice * tier5,
			[1, 13]
		);
		order = 300;
		new Game.Upgrade(
			...
			Game.Objects['Farm'].basePrice * tier5,
			[2, 13]
		);
		order = 400;
		new Game.Upgrade(
			...
			Game.Objects['Factory'].basePrice * tier5,
			[4, 13]
		);
		order = 500;
		new Game.Upgrade(
			...
			Game.Objects['Mine'].basePrice * tier5,
			[3, 13]
		);
		order = 600;
		new Game.Upgrade(
			...
			Game.Objects['Shipment'].basePrice * tier5,
			[5, 13]
		);
		order = 700;
		new Game.Upgrade(
			...
			Game.Objects['Alchemy lab'].basePrice * tier5,
			[6, 13]
		);
		order = 800;
		new Game.Upgrade(
			...
			Game.Objects['Portal'].basePrice * tier5,
			[7, 13]
		);
		order = 900;
		new Game.Upgrade(
			...
			Game.Objects['Time machine'].basePrice * tier5,
			[8, 13]
		);
		order = 1000;
		new Game.Upgrade(
			...
			Game.Objects['Antimatter condenser'].basePrice * tier5,
			[13, 13]
		);
		...
		new Game.Upgrade(
			'Caramoas',
			"Cookie production multiplier <b>+25%</b>.<q>Yeah. That's got a nice ring to it.</q>",
			999999999999999,
			[14, 4]
		);
		new Game.Upgrade(
			'Sagalongs',
			"Cookie production multiplier <b>+25%</b>.<q>Grandma's favorite?</q>",
			999999999999999,
			[15, 3]
		);
		new Game.Upgrade(
			'Shortfoils',
			'Cookie production multiplier <b>+25%</b>.<q>Foiled again!</q>',
			999999999999999,
			[15, 4]
		);
		new Game.Upgrade(
			'Win mints',
			"Cookie production multiplier <b>+25%</b>.<q>They're the luckiest cookies you've ever tasted!</q>",
			999999999999999,
			[14, 3]
		);
		type = '';
		...
		new Game.Upgrade(
			'Fig gluttons',
			'Cookie production multiplier <b>+25%</b>.<q>Got it all figured out.</q>',
			999999999999999,
			[17, 4]
		);
		new Game.Upgrade(
			'Loreols',
			"Cookie production multiplier <b>+25%</b>.<q>Because, uh... they're worth it?</q>",
			999999999999999,
			[16, 3]
		);
		new Game.Upgrade(
			'Jaffa cakes',
			'Cookie production multiplier <b>+25%</b>.<q>If you want to bake a cookie from scratch, you must first build a factory.</q>',
			999999999999999,
			[17, 3]
		);
		new Game.Upgrade(
			"Grease's cups",
			'Cookie production multiplier <b>+25%</b>.<q>Extra-greasy peanut butter.</q>',
			999999999999999,
			[16, 4]
		);
		type = '';
		...
		new Game.Upgrade(
			...
			"Unlocks <b>5%</b> of the potential of your heavenly chips.<q>Grants the knowledge of heavenly chips, and how to use them to make baking more efficient.<br>It's a secret to everyone.</q>",
			11,
			[19, 7]
		);
		new Game.Upgrade(
			'Heavenly cookie stand',
			"Unlocks <b>25%</b> of the potential of your heavenly chips.<q>Don't forget to visit the heavenly lemonade stand afterwards.</q>",
			1111,
			[18, 7]
		);
		new Game.Upgrade(
			'Heavenly bakery',
			'Unlocks <b>50%</b> of the potential of your heavenly chips.<q>Also sells godly cakes and divine pastries.</q>',
			111111,
			[17, 7]
		);
		new Game.Upgrade(
			'Heavenly confectionery',
			'Unlocks <b>75%</b> of the potential of your heavenly chips.<q>They say angel bakers work there. They take angel lunch breaks and sometimes go on angel strikes.</q>',
			11111111,
			[16, 7]
		);
		new Game.Upgrade(
			'Heavenly key',
			'Unlocks <b>100%</b> of the potential of your heavenly chips.<q>This is the key to the pearly (and tasty) gates of pastry heaven, granting you access to your entire stockpile of heavenly chips.<br>May you use them wisely.</q>',
			1111111111,
			[15, 7]
		);
		...
		new Game.Upgrade(
			'Skull cookies',
			"Cookie production multiplier <b>+20%</b>.<q>Wanna know something spooky? You've got one of these inside your head RIGHT NOW.</q>",
			444444444444,
			[12, 8]
		);
		new Game.Upgrade(
			'Ghost cookies',
			"Cookie production multiplier <b>+20%</b>.<q>They're something strange, but they look pretty good!</q>",
			444444444444,
			[13, 8]
		);
		new Game.Upgrade(
			'Bat cookies',
			'Cookie production multiplier <b>+20%</b>.<q>The cookies this town deserves.</q>',
			444444444444,
			[14, 8]
		);
		new Game.Upgrade(
			'Slime cookies',
			'Cookie production multiplier <b>+20%</b>.<q>The incredible melting cookies!</q>',
			444444444444,
			[15, 8]
		);
		new Game.Upgrade(
			'Pumpkin cookies',
			'Cookie production multiplier <b>+20%</b>.<q>Not even pumpkin-flavored. Tastes like glazing. Yeugh.</q>',
			444444444444,
			[16, 8]
		);
		new Game.Upgrade(
			'Eyeball cookies',
			'Cookie production multiplier <b>+20%</b>.<q>When you stare into the cookie, the cookie stares back at you.</q>',
			444444444444,
			[17, 8]
		);
		new Game.Upgrade(
			'Spider cookies',
			'Cookie production multiplier <b>+20%</b>.<q>You found the recipe on the web. They do whatever a cookie can.</q>',
			444444444444,
			[18, 8]
		);
		type = '';
		...
		new Game.Upgrade(
			'Christmas tree biscuits',
			'Cookie production multiplier <b>+20%</b>.<q>Whose pine is it anyway?</q>',
			252525252525,
			[12, 10]
		);
		new Game.Upgrade(
			'Snowflake biscuits',
			'Cookie production multiplier <b>+20%</b>.<q>Mass-produced to be unique in every way.</q>',
			252525252525,
			[13, 10]
		);
		new Game.Upgrade(
			'Snowman biscuits',
			"Cookie production multiplier <b>+20%</b>.<q>It's frosted. Doubly so.</q>",
			252525252525,
			[14, 10]
		);
		new Game.Upgrade(
			'Holly biscuits',
			"Cookie production multiplier <b>+20%</b>.<q>You don't smooch under these ones. That would be the mistletoe (which, botanically, is a smellier variant of the mistlefinger).</q>",
			252525252525,
			[15, 10]
		);
		new Game.Upgrade(
			'Candy cane biscuits',
			"Cookie production multiplier <b>+20%</b>.<q>It's two treats in one!<br>(Further inspection reveals the frosting does not actually taste like peppermint, but like mundane sugary frosting.)</q>",
			252525252525,
			[16, 10]
		);
		new Game.Upgrade(
			'Bell biscuits',
			'Cookie production multiplier <b>+20%</b>.<q>What do these even have to do with christmas? Who cares, ring them in!</q>',
			252525252525,
			[17, 10]
		);
		new Game.Upgrade(
			'Present biscuits',
			'Cookie production multiplier <b>+20%</b>.<q>The prequel to future biscuits. Watch out!</q>',
			252525252525,
			[18, 10]
		);
		type = '';
		...
		new Game.Upgrade(
			'Gingerbread men',
			'Cookie production multiplier <b>+25%</b>.<q>You like to bite the legs off first, right? How about tearing off the arms? You sick monster.</q>',
			9999999999999999,
			[18, 4]
		);
		new Game.Upgrade(
			'Gingerbread trees',
			'Cookie production multiplier <b>+25%</b>.<q>Evergreens in pastry form. Yule be surprised what you can come up with.</q>',
			9999999999999999,
			[18, 3]
		);
		type = '';
		...
		new Game.Upgrade(
			'Increased merriness',
			"Cookie production multiplier <b>+15%</b>.<q>It turns out that the key to increased merriness, strangely enough, happens to be a good campfire and some s'mores.<br>You know what they say, after all; the s'more, the merrier.</q>",
			2525,
			[17, 9]
		);
		new Game.Upgrade(
			'Improved jolliness',
			'Cookie production multiplier <b>+15%</b>.<q>A nice wobbly belly goes a long way.<br>You jolly?</q>',
			2525,
			[17, 9]
		);
		new Game.Upgrade(
			'A lump of coal',
			"Cookie production multiplier <b>+1%</b>.<q>Some of the world's worst stocking stuffing.<br>I guess you could try starting your own little industrial revolution, or something?...</q>",
			2525,
			[13, 9]
		);
		new Game.Upgrade(
			'An itchy sweater',
			"Cookie production multiplier <b>+1%</b>.<q>You don't know what's worse : the embarrassingly quaint \"elf on reindeer\" motif, or the fact that wearing it makes you feel like you're wrapped in a dead sasquatch.</q>",
			2525,
			[14, 9]
		);
		new Game.Upgrade(
			'Reindeer baking grounds',
			'Reindeer appear <b>twice as frequently</b>.<q>Male reindeer are from Mars; female reindeer are from venison.</q>',
			2525,
			[12, 9]
		);
		new Game.Upgrade(
			'Weighted sleighs',
			'Reindeer are <b>twice as slow</b>.<q>Hope it was worth the weight.<br>(Something something forced into cervidude)</q>',
			2525,
			[12, 9]
		);
		new Game.Upgrade(
			'Ho ho ho-flavored frosting',
			"Reindeer give <b>twice as much</b>.<q>It's time to up the antler.</q>",
			2525,
			[12, 9]
		);
		new Game.Upgrade(
			'Season savings',
			"All buildings are <b>1% cheaper</b>.<q>By Santa's beard, what savings!<br>But who will save us?</q>",
			2525,
			[16, 9],
			function () {
				Game.RefreshBuildings();
		...
		new Game.Upgrade(
			'Toy workshop',
			'All upgrades are <b>5% cheaper</b>.<q>Watch yours-elf around elvesdroppers who might steal our production secrets.<br>Or elven worse!</q>',
			2525,
			[16, 9],
			function () {
				Game.upgradesToRebuild = 1;
		...
		new Game.Upgrade(
			'Naughty list',
			"Grandmas are <b>twice</b> as productive.<q>This list contains every unholy deed perpetuated by grandmakind.<br>He won't be checking this one twice.<br>Once. Once is enough.</q>",
			2525,
			[15, 9]
		);
		new Game.Upgrade(
			"Santa's bottomless bag",
			"Random drops are <b>10% more common</b>.<q>This is one bottom you can't check out.</q>",
			2525,
			[19, 9]
		);
		new Game.Upgrade(
			"Santa's helpers",
			'Clicking is <b>10% more powerful</b>.<q>Some choose to help hamburger; some choose to help you.<br>To each their own, I guess.</q>',
			2525,
			[19, 9]
		);
		new Game.Upgrade(
			"Santa's legacy",
			"Cookie production multiplier <b>+10% per Santa's levels</b>.<q>In the north pole, you gotta get the elves first. Then when you get the elves, you start making the toys. Then when you get the toys... then you get the cookies.</q>",
			2525,
			[19, 9]
		);
		new Game.Upgrade(
			"Santa's milk and cookies",
			"Milk is <b>5% more powerful</b>.<q>Part of Santa's dreadfully unbalanced diet.</q>",
			2525,
			[19, 9]
		);
		...
		new Game.Upgrade(
			"Santa's dominion",
			'Cookie production multiplier <b>+50%</b>.<br>All buildings are <b>1% cheaper</b>.<br>All upgrades are <b>2% cheaper</b>.<q>My name is Claus, king of kings;<br>Look on my toys, ye Mighty, and despair!</q>',
			2525252525252525,
			[19, 10],
			function () {
				Game.RefreshBuildings();
		...
		new Game.Upgrade(
			'Pure heart biscuits',
			'Cookie production multiplier <b>+25%</b>.<q>Melty white chocolate<br>that says "I *like* like you".</q>',
			9999999999999999,
			[19, 3]
		);
		new Game.Upgrade(
			'Ardent heart biscuits',
			'Cookie production multiplier <b>+25%</b>.<q>A red hot cherry biscuit that will nudge the target of your affection in interesting directions.</q>',
			9999999999999999,
			[20, 3]
		);
		new Game.Upgrade(
			'Sour heart biscuits',
			'Cookie production multiplier <b>+25%</b>.<q>A bitter lime biscuit for the lonely and the heart-broken.</q>',
			9999999999999999,
			[20, 4]
		);
		new Game.Upgrade(
			'Weeping heart biscuits',
			'Cookie production multiplier <b>+25%</b>.<q>An ice-cold blueberry biscuit, symbol of a mending heart.</q>',
			9999999999999999,
			[21, 3]
		);
		new Game.Upgrade(
			'Golden heart biscuits',
			'Cookie production multiplier <b>+25%</b>.<q>A beautiful biscuit to symbolize kindness, true love, and sincerity.</q>',
			9999999999999999,
			[21, 4]
		);
		new Game.Upgrade(
			'Eternal heart biscuits',
			"Cookie production multiplier <b>+25%</b>.<q>Silver icing for a very special someone you've liked for a long, long time.</q>",
			9999999999999999,
			[19, 4]
		);
		type = '';
		...
		new Game.Upgrade(
			'Gem polish',
			'Prisms gain <b>+1,000,000</b> base CpS.<q>Get rid of the grime and let more light in.<br>Truly, truly outrageous.</q>',
			Game.Objects['Prism'].basePrice * tier1,
			[14, 0]
		);
		new Game.Upgrade(
			'9th color',
			"Prisms are <b>twice</b> as efficient.<q>Delve into untouched optical depths where even the mantis shrimp hasn't set an eye!</q>",
			Game.Objects['Prism'].basePrice * tier2,
			[14, 0]
		);
		new Game.Upgrade(
			'Chocolate light',
			...
			Game.Objects['Prism'].basePrice * tier3,
			[14, 1]
		);
		new Game.Upgrade(
			'Grainbow',
			...
			Game.Objects['Prism'].basePrice * tier4,
			[14, 2]
		);
		new Game.Upgrade(
			'Pure cosmic light',
			...
			Game.Objects['Prism'].basePrice * tier5,
			[14, 13]
		);
		...
		new Game.Upgrade(
			'Rainbow grandmas',
			'Grandmas are <b>twice</b> as efficient.<q>A luminous grandma to sparkle into cookies.</q>',
			Game.Objects['Prism'].basePrice * tier2,
			[10, 9],
			function () {
				Game.Objects['Grandma'].drawFunction();
		...
		Game.computeSeasonPrices = function () {
			for (var i in Game.seasonTriggers) {
				Game.Upgrades[Game.seasonTriggers[i]].basePrice =
					Game.seasonTriggerBasePrice * Math.pow(10, Game.seasonUses);
			}
		};
		new Game.Upgrade(
			'Season switcher',
		...
		new Game.Upgrade(
			'Festive biscuit',
			"Triggers <b>Christmas season</b> for the next 24 hours.<br>Triggering another season will cancel this one.<q>'Twas the night before Christmas- or was it?</q>",
		...
		new Game.Upgrade(
			'Ghostly biscuit',
			'Triggers <b>Halloween season</b> for the next 24 hours.<br>Triggering another season will cancel this one.<q>spooky scary skeletons<br>will wake you with a boo</q>',
			Game.seasonTriggerBasePrice,
			[13, 8]
		);
		...
		new Game.Upgrade(
			'Lovesick biscuit',
			"Triggers <b>Valentine's Day season</b> for the next 24 hours.<br>Triggering another season will cancel this one.<q>Romance never goes out of fashion.</q>",
			Game.seasonTriggerBasePrice,
			[20, 3]
		);
		...
		*/
		...
		new Game.Achievement(
			'Speed baking I',
			'Get to <b>1 million</b> cookies baked in <b>35 minutes</b> (with no heavenly upgrades).',
			[12, 5],
			3
		);
		new Game.Achievement(
			'Speed baking II',
			'Get to <b>1 million</b> cookies baked in <b>25 minutes</b> (with no heavenly upgrades).',
			[13, 5],
			3
		);
		new Game.Achievement(
			'Speed baking III',
			'Get to <b>1 million</b> cookies baked in <b>15 minutes</b> (with no heavenly upgrades).',
			[14, 5],
			3
		);
		...
		new Game.Achievement(
			'Spooky cookies',
			'Unlock <b>every Halloween-themed cookie</b>.<br>Owning this achievement makes Halloween-themed cookies drop more frequently in future playthroughs.',
			[12, 8],
			2
		);
		...
		new Game.Achievement(
			'Let it snow',
			'Unlock <b>every Christmas-themed cookie</b>.',
			[19, 9],
			2
		);
		...
		new Game.Achievement(
			'Lord of Constructs',
			'Own <b>1500</b> buildings.<q>He saw the vast plains stretching ahead of him, and he said : let there be civilization.</q>',
			[6, 6],
			2
		);
		order = 6000;
		new Game.Achievement(
			'Lord of Progress',
			'Purchase <b>150</b> upgrades.<q>One can always do better. But should you?</q>',
			[9, 2],
			2
		);
		order = 7000;
		...
		Game.UpdateGrandmapocalypse = function () {
			...
			Game.elderWrathD +=
				(Game.elderWrath + 1 - Game.elderWrathD) * 0.001; //slowly fade to the target wrath state
		...
		Game.UpdateWrinklers = function () {
			...
			for (var i in Game.wrinklers) {
				...
				if (
					...
					Game.elderWrath > 0 &&
					Math.random() <
						(!Game.Has('Wrinkler doormat') ? 0.0001 : 0.1)
				) {
					me.phase = 1;
					me.hp = 3;
				} //respawn
				...
				if (me.phase > 0) {
					...
					if (
						...
						if (Game.Click) {
							...
							me.hp--;
							var x =
								me.x + Math.sin((me.r * Math.PI) / 180) * 100;
							var y =
								me.y + Math.cos((me.r * Math.PI) / 180) * 100;
		...
		Game.DrawWrinklers = function () {
			for (var i in Game.wrinklers) {
				...
				if (me.phase > 0) {
					...
					if (Game.season == 'christmas')
						Game.LeftBackground.drawImage(
							Game.Assets['winterWrinkler.png'],
							-50,
							-10
						);
					else
						Game.LeftBackground.drawImage(
							Game.Assets['wrinkler.png'],
							-50,
							-10
						);
		...
		Game.UpdateSanta = function () {
			if (Game.LeftBackground) {
				...
				if (
					Math.abs(Game.mouseX - x) < 48 &&
					Math.abs(Game.mouseY - y) < 48
				) {
					Game.mousePointer = 1;
					if (Game.Click) {
						var moni = Math.pow(
						...
						if (Game.cookies > moni && Game.santaLevel < 14) {
							...
							if (Game.santaLevel == 14) {
								...
								Game.Popup(
									"You are granted<br>Santa's dominion."
								);
							}
							...
							if (drop) {
								Game.Unlock(drop);
								Game.Popup(
									'You find a present which contains...<br>' +
										drop +
										'!'
								);
							}
				...
				if (Game.santaTransition > 0) {
					...
					if (Game.santaTransition >= Game.fps / 2)
						Game.santaTransition = 0;
				}
			}
		};
		Game.DrawSanta = function () {
			...
			Game.LeftBackground.save();
			Game.LeftBackground.translate(
				x + Math.cos(Game.T * 0.2) * 4,
				y + 48
			);
			Game.LeftBackground.rotate(r);
			Game.LeftBackground.drawImage(
				Game.Assets['santa.png'],
				96 * Game.santaLevel,
				0,
				96,
				96,
			...
			if (Game.santaTransition > 0) {
				Game.LeftBackground.globalAlpha =
					1 - Game.santaTransition / (Game.fps / 2);
				var s = 1 + Game.santaTransition / (Game.fps / 2);
				Game.LeftBackground.drawImage(
					Game.Assets['santa.png'],
					96 * Math.max(0, Game.santaLevel - 1),
					0,
					96,
					96,
					-48 * s,
		...
		Game.DrawBackground = function () {
			...
			if (!Game.Background) {
				...
				Game.LeftBackground = l('backgroundLeftCanvas').getContext(
					'2d'
				);
			...
			Game.cookieOriginX = Math.floor(
				Game.LeftBackground.canvas.width / 2
			);
			Game.cookieOriginY = Math.floor(
				Game.LeftBackground.canvas.height * 0.4
			);
			...
			if (Game.prefs.particles) {
				...
				if (pic != '') {
					if (Game.elderWrathD >= 1)
						opacity =
							1 - (Math.min(Game.elderWrathD, 1.5) - 1) / 0.5;
					Game.LeftBackground.globalAlpha = opacity;
					var y = Math.floor(Game.T * 2) % 512;
					Game.LeftBackground.fillPattern(
						Game.Assets[pic],
						0,
						-512 + y,
						Game.LeftBackground.canvas.width,
						Game.LeftBackground.canvas.height + 512,
						512,
						512
					);
					Game.LeftBackground.globalAlpha = 1;
				...
				if (Game.season == 'christmas') {
					...
					Game.LeftBackground.globalCompositeOperation =
						'source-over';
					Game.LeftBackground.globalAlpha = 1;
				}
				//hearts
			...
			if (Game.prefs.milk) {
				...
				var y =
					Game.milkHd *
					Game.LeftBackground.canvas.height *
					(1 + 0.05 * (Math.sin(Game.T * 0.017) / 2 + 0.5));
				/*
				var m1o=1;
				var m2o=0;
				var m1i=0;
				...
				*/
				...
				Game.LeftBackground.fillPattern(
					Game.Assets[pic + '.png'],
					-480 + x,
					Game.LeftBackground.canvas.height - y,
					Game.LeftBackground.canvas.width + 480,
					1,
					480,
					480
				);
				...
				Game.LeftBackground.fillRect(
					0,
					Game.LeftBackground.canvas.height - y + 480,
					Game.LeftBackground.canvas.width,
					Math.max(0, y - 480)
				);
				Game.LeftBackground.globalAlpha = 1;
			...
			if (
				Game.clickFrenzy > 0 ||
				(Game.frenzy > 0 && Game.frenzyPower >= 1)
			)
				borders = 'shadedBordersGold.png';
			...
			if (Game.frenzy > 0 && Game.frenzyPower >= 100) {
				Game.timersEl['elderFrenzy'].style.width =
					(Game.frenzy / Game.frenzyMax) * 100 + '%';
				Game.timersEl['elderFrenzy'].style.display = 'block';
			} else if (Game.frenzy > 0 && Game.frenzyPower > 1) {
				Game.timersEl['frenzy'].style.width =
					(Game.frenzy / Game.frenzyMax) * 100 + '%';
				Game.timersEl['frenzy'].style.display = 'block';
			} else if (Game.frenzy > 0 && Game.frenzyPower < 1) {
				Game.timersEl['clot'].style.width =
					(Game.frenzy / Game.frenzyMax) * 100 + '%';
				Game.timersEl['clot'].style.display = 'block';
			}
			if (Game.clickFrenzy > 0) {
				Game.timersEl['clickFrenzy'].style.width =
					(1 - Game.clickFrenzy / Game.clickFrenzyMax) * 100 + '%';
	...
	Game.Logic = function () {
		...
		if (Game.T % (Game.fps * 5) == 0 && Game.ObjectsById.length > 0) {
			...
			if (
				...
				if (Game.Has('Empire biscuits'))
					Game.Unlock('British tea biscuits');
			...
			if (Game.cookiesEarned >= 9999999999999) {
				...
				if (Game.prestige['Heavenly chips'] >= 1)
					Game.Unlock('Caramoas');
				if (Game.prestige['Heavenly chips'] >= 2)
					Game.Unlock('Sagalongs');
				if (Game.prestige['Heavenly chips'] >= 3)
					Game.Unlock('Shortfoils');
				if (Game.prestige['Heavenly chips'] >= 4)
					Game.Unlock('Win mints');

				if (Game.prestige['Heavenly chips'] >= 10)
					Game.Unlock('Fig gluttons');
				if (Game.prestige['Heavenly chips'] >= 100)
					Game.Unlock('Loreols');
				if (Game.prestige['Heavenly chips'] >= 500)
					Game.Unlock('Jaffa cakes');
				if (Game.prestige['Heavenly chips'] >= 2000)
					Game.Unlock("Grease's cups");

				if (Game.prestige['Heavenly chips'] >= 5000)
					Game.Unlock('Season switcher');
			}
			if (Game.cookiesEarned >= 99999999999999) {
				Game.Unlock(['Gingerbread men', 'Gingerbread trees']);
				if (Game.season == 'valentines') {
					Game.Unlock('Pure heart biscuits');
					if (Game.Has('Pure heart biscuits'))
						Game.Unlock('Ardent heart biscuits');
					if (Game.Has('Ardent heart biscuits'))
						Game.Unlock('Sour heart biscuits');
					if (Game.Has('Sour heart biscuits'))
						Game.Unlock('Weeping heart biscuits');
					if (Game.Has('Weeping heart biscuits'))
						Game.Unlock('Golden heart biscuits');
					if (Game.Has('Golden heart biscuits'))
						Game.Unlock('Eternal heart biscuits');
				}
			}
				...
				if (!Game.HasAchiev('Mathematician')) {
					if (
						...
						Math.min(
							...
							Math.pow(
								2,
								Game.ObjectsById.length - Game.Objects[i].id - 1
							)
						)
					)
						mathematician = 0;
			...
			if (Game.UpgradesOwned == 0 && Game.cookiesEarned >= 1000000000)
				Game.Win('Hardcore');
		...
		else if (Game.veil == 0 && Game.cookiesEarned < veilLimit)
			Game.veilOn();
	...
	Game.Draw = function () {
		...
		if (Math.round(Game.cookiesd).toString().length > 11 && !Game.mobile)
			unit = '<br>cookies';
		var str =
		...
		if (Game.drawT % 30 == 0) {
			...
			l(
				'productIcon' + Game.Objects['Grandma'].id
			).style.backgroundImage =
				'url(img/' + grandmaIcons[Game.elderWrath] + '.png)'; //oh god fix this (should change when the store tabs are updated and when the grandmapocalypse level changes)
		}
	...
	Game.Loop = function () {
		...
		//latency compensator
		Game.accumulatedDelay +=
			new Date().getTime() - Game.time - 1000 / Game.fps;
